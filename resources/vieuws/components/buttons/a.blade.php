...
<a {{ $href ?? '' }} class="flex items-center justify-center font-semibold outline-none transition duration-150 rounded-full gap-1.5 py-4 px-8 text-sm shadow-sm focus:outline-none hover:transform hover:scale-[1.01] {{ $typeClass }} {{ $attributes->get('class') }} @if($disabled) cursor-not-allowed @endif"
